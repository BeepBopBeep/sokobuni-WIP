...
export class Migration20250915090937 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "ticket_purchase" ("id" text not null, "order_id" text not null, "ticket_product_id" text not null, "ticket_variant_id" text not null, "venue_row_id" text not null, "seat_number" text not null, "show_date" timestamptz not null, "status" text check ("status" in ('pending', 'scanned')) not null default 'pending', "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "ticket_purchase_pkey" primary key ("id"));`);
